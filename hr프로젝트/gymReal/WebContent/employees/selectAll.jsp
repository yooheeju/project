...
<h3><b>인사 관리</b></h3>
		...
		<div class="search-container">
			<button class="event-button"
				onclick="sortTable('employees', 3, 'asc')">오름차순</button>
			<button class="event-button"
				onclick="sortTable('employees', 3, 'desc')">내림차순</button>
		</div>
	</div>
	...
	<div style="text-align: center; max-width: 1000px; margin: 0 auto;">
		<table width="100%" id="employees" class="bordered">
			<tr>
				<th>센터 No.</th>
				<th>분야 No.</th>
				<th>직원 ID</th>
			...
			<c:forEach items="${dtos}" var="employeesDto">
				<tr>
					<td>${employeesDto.center_id}</td>
					<td>${employeesDto.job_id}</td>
					<td><a
						href='/gymReal/employees/selectOne.employees?employee_id=${employeesDto.employee_id}'>
							${employeesDto.employee_id}</a></td>
					<td>${employeesDto.name}</td>
					<td>${employeesDto.email}</td>
					<td>${employeesDto.phone}</td>
